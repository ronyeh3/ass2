...
public abstract class MicroService implements Runnable {
	...
	ConcurrentHashMap<Class<? extends Message>,Callback> fCallbacksForRequestsAndBroadcasts; 
	ConcurrentHashMap<Request<?>, Callback> fCallbacksForRequestCompleted;       
	...
	public MicroService(String name) {         
		this.fName = name;
		fCallbacksForRequestsAndBroadcasts = new ConcurrentHashMap<Class<? extends Message>,Callback>();
		fCallbacksForRequestCompleted = new ConcurrentHashMap<Request<?>,Callback>();
	...
	protected final <R extends Request> void subscribeRequest(Class<R> type, Callback<R> callback) {
		...
		fCallbacksForRequestsAndBroadcasts.put(type, callback);
	}
	...
	protected final <T> boolean sendRequest(Request<T> r, Callback<T> onComplete) {
		if (!fCallbacksForRequestCompleted.contains(r.getClass())){
			fCallbacksForRequestCompleted.put(r, onComplete);            
		}
	...
	protected final <T> void complete(Request<T> r, T result) { 
    	...
    	fCallbacksForRequestCompleted.remove(r); //deep understanding of callbacks
    }
	...
	public final void run() {
		...
		while (!fTerminated) {
			try {
				Message recivedMessage=MessageBusImpl.getInstance().awaitMessage(this);
				if(recivedMessage.getClass()==RequestCompleted.class){
					Request completedMessage = RequestCompleted.class.cast(recivedMessage).getCompletedRequest();
					fCallbacksForRequestCompleted.get(completedMessage).call( RequestCompleted.class.cast(recivedMessage).getResult()) ;
				}
				else fCallbacksForRequestsAndBroadcasts.get(recivedMessage.getClass()).call(recivedMessage);
			}
